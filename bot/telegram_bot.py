...
class ChatGPTTelegramBot:
    ...
    async def set_assistant_prompt(
        ...
        new_assistant_prompt = message_text(update.message)
        if new_assistant_prompt == "":
            await update.effective_message.reply_text(
                message_thread_id=get_thread_id(update),
                text="new_assistant_prompt is empty",
            )
            return

        self.openai.config["assistant_prompt"] = new_assistant_prompt
        logging.info(
            ...
            json.dump(
                {
                    ...
                    "VOICE_REPLY_PROMPTS": self.openai.config["voice_reply_prompts"],
                },
                f,
                ensure_ascii=False,
                indent=2,
    ...
    async def set_vision_prompt(
        ...
        new_vision_prompt = message_text(update.message)
        if new_vision_prompt == "":
            await update.effective_message.reply_text(
                message_thread_id=get_thread_id(update),
                text="new_vision_prompt is empty",
            )
            return
    ...
    async def set_whisper_prompt(
        ...
        new_whisper_prompt = message_text(update.message)
        if new_whisper_prompt == "":
            await update.effective_message.reply_text(
                message_thread_id=get_thread_id(update),
                text="new_whisper_prompt is empty",
            )
            return
    ...
    def run(self):
        ...
        application.add_handler(
            CommandHandler("set_assistant_prompt", self.set_assistant_prompt)
        )
        application.add_handler(
            CommandHandler("get_vision_prompt", self.get_vision_prompt)
        )
        application.add_handler(
            CommandHandler("set_vision_prompt", self.set_vision_prompt)
        )
        application.add_handler(
            CommandHandler("get_whisper_prompt", self.get_whisper_prompt)
        )
        application.add_handler(
            CommandHandler("set_whisper_prompt", self.set_whisper_prompt)
        )
        application.add_handler(
            CommandHandler(
                "chat",
